...
# Wechselkurs & Ticker Info vorab
usd_ticker = yf.Ticker("EURUSD=X")
eur_usd_rate = usd_ticker.info.get('regularMarketPrice', 1.09)
...
if mode == "Analyse" and st.session_state.search_query:
    try:
        ...
        if not df_full.empty:
            ...
            perf_abs = (end_price - start_price) * eur_to_usd_factor

            # Kennzahlen Zeile 1
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Preis (€)", f"{end_price * eur_to_usd_factor:.2f} €")
            m2.metric("Preis ($)", f"{end_price:.2f} $")
            m3.metric(f"Performance ({st.session_state.period})", f"{perf_pct:.2f} %", delta=f"{perf_abs:.2f} €")
            ...
            m4.metric("RSI (14)", f"{df_full['RSI'].iloc[-1]:.1f}")

            # Kennzahlen Zeile 2
            m5, m6, m7, m8 = st.columns(4)
            m5.metric("KGV", f"{info.get('forwardPE', 'N/A')}")
            m6.metric("Dividende", f"{(info.get('dividendYield', 0) or 0)*100:.2f} %")
            rating = info.get('recommendationMean', 3.0)
            m7.metric("Rating", "👍 Strong Buy" if rating <= 2.1 else "✅ Buy" if rating <= 2.6 else "➡️ Hold")
            m8.metric("Börse", info.get('exchange', 'N/A'))
            ...
            fig = make_subplots(rows=3, cols=1, shared_xaxes=True, 
                                ...
                                subplot_titles=("Kursverlauf", "Volumen", "RSI (14)"))
    ...
    with st.spinner("Lade Daten..."):
        df_sb = get_industry_recommendations(selected_industry, eur_to_usd_factor)
        if not df_sb.empty:
            c1, c2 = st.columns(2)
